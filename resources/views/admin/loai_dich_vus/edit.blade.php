        ...
        <div class="rounded-sm border border-stroke bg-white shadow-default p-4">
            <div class="border-b border-stroke py-4 px-6.5">
                <h3 class="font-semibold text-black">
                    Thay đổi thông tin loại dịch vụ
                </h3>
            </div>
            <form action="{{ route('loai_dich_vus.update', $loai_dich_vu) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="p-6.5">
                    <div class="mb-6 flex flex-col gap-6 xl:flex-row">
                        <div class="w-full xl:w-1/2">
                            <label class="mb-6 block text-black font-bold">
                                Tên loại dịch vụ <span class="text-meta-1">*</span>
                            </label>
                            <input name="tenLoai" value="{{ old('tenLoai', $loai_dich_vu->tenLoai) }}" type="text" placeholder="Nhập tên loại dịch vụ" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter">
