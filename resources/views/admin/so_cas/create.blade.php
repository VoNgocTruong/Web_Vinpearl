...
@section('content')
    ...
    <div class="w-2/4 mx-auto pt-2">
        <div class="rounded-sm border border-stroke bg-white shadow-default p-4">
            ...
            <form action="{{ route('so_cas.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="p-6.5">
                    <div class="mb-6">
                        ...
                        <div class="relative z-20 bg-transparent">
                            <select name="maNV" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary">
                                @foreach($nhan_viens as $nv)
                                    <option value="{{ $nv->maNV }}" @if(old('maNV') == $nv->maNV) selected @endif>{{ $nv->hoTenNV }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-6 flex flex-col gap-6 xl:flex-row">
                        <div class="w-full xl:w-1/2">
                            ...
                            <input name="soCa" value="{{ old('soCa') }}" type="text" placeholder="Nhập số Ca" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter">
                            @if($errors->has('soCa'))
