...
@section('content')
    ...
    <div class="w-2/4 mx-auto pt-2">
        <div class="rounded-sm border border-stroke bg-white shadow-default p-4">
            ...
            <form action="{{ route('dich_vus.update', $dich_vu) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="p-6.5">
                    <div class="mb-4.5 flex flex-col gap-6 xl:flex-row">
                        <div class="w-full xl:w-1/2">
                            ...
                            <input name="tenDV" value="{{ old('tenDV', $dich_vu->tenDV) }}" type="text" placeholder="Nhập tên dịch vụ" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter">
                            @if($errors->has('tenDV'))
                        ...
                        <div class="w-full xl:w-1/2">
                            ...
                            <input name="sdtDV" value="{{ old('sdtDV', $dich_vu->sdtDV) }}" type="text" placeholder="Nhập số điện thoại" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter">
                            @if($errors->has('sdtDV'))
                    ...
                    <div class="mb-4.5 flex flex-col gap-6 xl:flex-row">
                        <div class="w-full xl:w-1/2">
                            ...
                            <div>
                                <input value="{{ old('anh', $dich_vu->anh) }}" name="anh" type="file" class="w-full cursor-pointer rounded-lg border-[1.5px] border-stroke bg-transparent font-medium outline-none transition file:mr-5 file:border-collapse file:cursor-pointer file:border-0 file:border-r file:border-solid file:border-stroke file:bg-whiter file:py-3 file:px-5 file:hover:bg-primary file:hover:bg-opacity-10 focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter">
                                @if($errors->has('anh'))
                    ...
                    <div class="mb-6">
                        <label class="mb-2.5 block text-black font-bold">
                            Xếp Loại <span class="text-meta-1">*</span>
                        </label>
                        <input name="xepLoai" value="{{ old('xepLoai', $dich_vu->xepLoai) }}" type="text" placeholder="Nhập xếp loại" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="relative z-20 bg-transparent">
                            <select name="maLoaiDV" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary">
                                @foreach($loai_dich_vus as $ldv)
                                    <option value="{{ $ldv->maLoaiDV }}" {{ $dich_vu->maLoaiDV === $ldv->maLoaiDV ? 'selected' : '' }}>{{ $ldv->tenLoai }}</option>
                                @endforeach
                            </select>
                        ...
                        @if($errors->has('maLoaiDV'))
                            <span class="text-red-500">{{ $errors->first('maLoaiDV') }}</span>
                        @endif
                    </div>
