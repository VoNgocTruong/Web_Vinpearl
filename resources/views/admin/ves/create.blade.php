        ...
        <div class="rounded-sm border border-stroke bg-white shadow-default p-4">
            ...
            <form action="{{ route('ves.store') }}" method="post" enctype="multipart/form-data">
                ...
                <div class="p-6.5">
                        ...
                        <div class="relative z-20 bg-transparent">
                            <select name="maDV" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary">
                                @foreach($dich_vus as $dv)
                                    <option value="{{ $dv->maDV }}" @if(old('maDV') == $dv->maDV) selected @endif>{{ $dv->tenDV }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-6">
                        <label class="mb-2.5 block text-black font-bold">
                            Giá vé <span class="text-meta-1">*</span>
                        </label>
                        <input name="giaTien" value="{{ old('giaTien') }}" type="text" placeholder="Nhập giá vé" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter">
